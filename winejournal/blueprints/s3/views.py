...
def s3_signature():
    ...
    if response_data is None:
        response_data = {
            'error': 'This file has not been approved for transfer, check upload is to correct dataset.'}
    return jsonify(response_data)


# Probably delete this completely for systems that should't allow delete
@s3.route("/delete/<key>", methods=['POST', 'DELETE'])
@csrf.exempt
def s3_delete(key=None):
    ...
    print("\t**** THIS DATA USED TO NOTIFY BACKEND OF DELETED DATA ****",
          file=sys.stderr)
    print("\tBucket: %s\n\tKey: %s" % (
    request_payload.get('bucket'), request_payload.get('key')), file=sys.stderr)
    print("\t**********************************************************",
          file=sys.stderr)
...
@s3.route("/success", methods=['GET', 'POST'])
@csrf.exempt
def s3_success():
    """ Success redirect endpoint for <=IE9. """

    print("\t**** THIS DATA USED TO NOTIFY BACKEND OF COMPLETED DATA ****",
          file=sys.stderr)
